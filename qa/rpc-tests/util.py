...
# Copyright (c) 2014-2015 The Bolt developers
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    boltd and bolt-cli must be in search path.
    """

    if not os.path.isdir(os.path.join("cache", "node0")):
        devnull = open("/dev/null", "w+")
        # Create cache directories, run boltds:
        for i in range(4):
            datadir = os.path.join("cache", "node"+str(i))
            os.makedirs(datadir)
            with open(os.path.join(datadir, "bolt.conf"), 'w') as f:
                f.write("regtest=1\n");
                f.write("rpcuser=rt\n");
                f.write("rpcpassword=rt\n");
                f.write("port="+str(START_P2P_PORT+i)+"\n");
                f.write("rpcport="+str(START_RPC_PORT+i)+"\n");
            args = [ "boltd", "-keypool=1", "-datadir="+datadir ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(START_P2P_PORT))
            bitcoind_processes.append(subprocess.Popen(args))
            subprocess.check_call([ "bolt-cli", "-datadir="+datadir,
...
def start_nodes(num_nodes, dir):
    # Start boltds, and wait for RPC interface to be up and running:
    devnull = open("/dev/null", "w+")
    for i in range(num_nodes):
        datadir = os.path.join(dir, "node"+str(i))
        args = [ "boltd", "-datadir="+datadir ]
        bitcoind_processes.append(subprocess.Popen(args))
        subprocess.check_call([ "bolt-cli", "-datadir="+datadir,
                                  "-rpcwait", "getblockcount"], stdout=devnull)
    devnull.close()
