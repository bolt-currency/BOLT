    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat BOLT and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    if(!sessionFoundMasternode){
        ...
        LogPrintf("Submitting Darksend for %f BOLT CENT - sessionTotalValue %d\n", fDarkcoinSubmitted, sessionTotalValue);
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    if(!success){
       ...
       /* success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey,
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey,
                nFeeRet, nChangePos, strFail, coinControl, ONLY_NONDENOMINATED);
