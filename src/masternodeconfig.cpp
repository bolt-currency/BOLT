...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                strErr = "Invalid BOLT address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 11990) {
                strErr = "Invalid port detected in masternode.conf: " + line + " (must be 11990 for mainnet)";
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 11990) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (11990 must be only on mainnet)";
            streamConfig.close();
            return false;
        }
