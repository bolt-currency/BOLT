...
bool CWalletDB::WriteKey(const CPubKey& vchPubKey, const CPrivKey& vchPrivKey, const CKeyMetadata& keyMeta)
{
    ...
    return Write(std::make_pair(std::string("key"), vchPubKey), std::make_pair(vchPrivKey, HashBlake(vchKey.begin(), vchKey.end())), false);
}
    ...
    try {
        ...
        else if (strType == "key" || strType == "wkey")
        {
            ...
            if (hash != 0)
            {
                ...
                if (HashBlake(vchKey.begin(), vchKey.end()) != hash)
                {
