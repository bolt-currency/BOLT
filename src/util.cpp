...
//int64_t enforceMasternodePaymentsTime = 4085657524; // BOLT
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleBOLT(name.begin()+1, name.end());
            if (mapArgs.count(singleBOLT) == 0)
                mapArgs[singleBOLT] = entry.second;
            name = singleBOLT;
        }
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BOLT
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BOLT
    // Mac: ~/Library/Application Support/Bolt Wallet
    // Unix: ~/.bolt
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bolt Wallet";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bolt.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bolt.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bolt.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "boltd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BOLT will not work properly.");
