...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BOLTMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BOLTMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("BOLTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bolt-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running BOLTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BOLTMiner terminated\n");
        throw;
    }
}
