...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(addr.GetPort() != 11990) return;
        } else if(addr.GetPort() == 11990) return;
        ...
        if(AcceptableInputs(mempool, state, tx)){
            ...
            // should be at least not earlier than block when 1000 BOLT tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            GetTransaction(vin.prevout.hash, tx, hashBlock, true);
            map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second)
            {
                CBlockIndex* pMNIndex = (*mi).second; // block for 1000 BOLT tx -> 1 confirmation
