...
uint256 CBlock::BuildMerkleTree() const
{
    ...
    for (int nSize = vtx.size(); nSize > 1; nSize = (nSize + 1) / 2)
    {
        for (int i = 0; i < nSize; i += 2)
        {
            ...
            vMerkleTree.push_back(HashBlake(BEGIN(vMerkleTree[j+i]),  END(vMerkleTree[j+i]),
...
uint256 CBlock::CheckMerkleBranch(uint256 hash, const std::vector<uint256>& vMerkleBranch, int nIndex)
{
    ...
    {
        if (nIndex & 1)
            hash = HashBlake(BEGIN(otherside), END(otherside), BEGIN(hash), END(hash));
        else
            hash = HashBlake(BEGIN(hash), END(hash), BEGIN(otherside), END(otherside));
        nIndex >>= 1;
    }
    return hash;
}
