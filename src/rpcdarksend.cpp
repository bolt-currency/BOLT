...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <boltaddress> <amount>\n"
            "boltaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <boltaddress> <amount>\n"
            "boltaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in bolt.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  stop         - Stop masternode configured in bolt.conf\n"
                ...
                "  vote-many    - Vote on a BOLT initiative\n"
                "  vote         - Vote on a BOLT initiative\n"
                );
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "protocol" && strMode != "full" && strMode != "votes" && strMode != "donation" && strMode != "pose"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a BOLT initiative (can be additionally filtered, partial match)\n"
                );
    }
