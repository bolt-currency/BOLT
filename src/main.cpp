...
#if defined(NDEBUG)
# error "BOLT cannot be compiled without assertions."
#endif
...
static const int64_t nStartSubsidy = 15 * COIN;
...
int64_t GetBlockValue(int nBits, int nHeight, int64_t nFees)
{
    ...
    if(nHeight <= 40000){
        ...
        nSubsidy = nStartSubsidy/2;

    } else if(nHeight <= 160000) {

        nSubsidy = nStartSubsidy/3;
...
static const int64_t nTargetSpacing = 1 * 60; // 60 seconds
static const int64_t nInterval = 1; // BOLT: retarget every block
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    // current difficulty formula, bolt - DarkGravity v3, written by Evan Duffield - evan@boltpay.io 
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // BOLT: This fixes an issue where a 51% attack can change difficulty at will.
