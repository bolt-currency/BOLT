...
void static BIP32Hash(const unsigned char chainCode[32], unsigned int nChild, unsigned char header, const unsigned char data[32], unsigned char output[64]) {
    ...
    sph_blake512 (&ctx_blake, chainCode, 32);
    sph_blake512 (&ctx_blake, &header, 1);
    sph_blake512 (&ctx_blake, data, 32);
    sph_blake512 (&ctx_blake, num, 4);
...
void CExtKey::SetMaster(const unsigned char *seed, unsigned int nSeedLen) {
    ...
    uint512 hash1;
    sph_blake512_init(&ctx_blake);
    sph_blake512 (&ctx_blake, hashkey, sizeof(hashkey));
    sph_blake512 (&ctx_blake, seed, nSeedLen);
    

    unsigned char out[64];
    LockObject(out);

    sph_blake512_close(&ctx_blake, out);
